...
export const createIdentityDocumentTypeSchema = z.object({
  ...
  validationType: z.enum(['external_lookup', 'manual']).optional().default('manual'),
  docLength: z.number().int().positive().optional().nullable(),
  docPattern: z.string().max(100).optional().nullable(),
