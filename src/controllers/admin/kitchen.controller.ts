...
import * as kitchenService from '../../services/admin/kitchen.service';
...
export const getKitchenOrdersController = async (c: Context) => {
  try {
    ...
    const orders = await kitchenService.getActiveKitchenOrders(tenantId);
...
export const updateKitchenStatusController = async (c: Context) => {
  try {
    ...
    const result = await kitchenService.updateKitchenOrderStatus(id, tenantId, status as any);
